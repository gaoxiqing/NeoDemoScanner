    ...
    public class Class1 : IPlugin_Sync
    {
        ...
        public void OnSyncBlockTransAction(int block, MyJson.JsonNode_Object txjson)
        {
            ...
            foreach (var v in vout)
            {
                ...
                //if (address == "AdT3Lst1niDbmQceBWvUks7Uarx1Qtr6oH" || address== "AH2ADnKSuJrhHefqeJ9j83HcNXPfipwr6V"
                //    || address == "ALB9mBVNbLhXTEcNf1a9u9ExWGg3N6T4gq" || address == "Aege36GGHr7vPWePBX55PrSz81zLvC56yo")
                //{
        ...
        void MakeUTXO(string address, string txid, int n, string asset, string value)
        {
            ...
            money.SetDictValue("addr", address);
            money.SetDictValue("txid", txid);
            money.SetDictValue("n", n);
        }
        ...
        void SetTran(int block, string txid, string txtype, MyJson.JsonNode_Array vin, MyJson.JsonNode_Array vout)
        {
            ...
            var trans = new MyJson.JsonNode_Object();
            txmap[txid] = trans;
        ...
        public MyJson.IJsonNode RPC(string method, MyJson.JsonNode_Array args)
        {
            ...
            if (method == "getbalance")
            {
                ...
                foreach (var item in utxoaddr)
                {
                    if (item.Value.AsDict()["use"].AsString() == "")
                    {
                        balance += item.Value.AsDict()["value"].AsDouble();
                    }
                }
                outjson.SetDictValue("balance", balance);
                return outjson;
            }
            return null;
